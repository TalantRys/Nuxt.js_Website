<template>
  <footer>
    <div class="footer__container container">
      <nuxt-link to="/" exact class="footer__logo">Scake</nuxt-link>
        <p class="footer__copyright">@2022 ИСП306 Рыскулов Талант</p>
      <ul class="footer__list">
        <li><nuxt-link class="footer__link" to="#">Главная</nuxt-link></li>
        <li><nuxt-link class="footer__link" to="#">Торты</nuxt-link></li>
        <li><nuxt-link class="footer__link" to="#">О нас</nuxt-link></li>
      </ul>
      <ul class="footer__list footer__list_last">
        <li><nuxt-link class="footer__link" to="#">Доставка</nuxt-link></li>
        <li><nuxt-link class="footer__link" to="#">Связаться с нами</nuxt-link></li>
      </ul>
      <div class="footer__social">
        <p class="social__title">Мы в соц. сетях:</p>
        <div class="social__block">
          <img src="~assets/images/icons/Social/vk.svg" alt="vk" />
          <img src="~assets/images/icons/Social/instagram.svg" alt="instagram" />
        </div>
      </div>
      <div class="footer__pay">
        <p class="pay__title">Платёжные системы:</p>
        <div class="pay__block">
          <img src="~assets/images/icons/Pay/visa.svg" alt="visa" />
          <img src="~assets/images/icons/Pay/mastercard.svg" alt="mastercard" />
          <img src="~assets/images/icons/Pay/maestro.svg" alt="maestro" />
          <img src="~assets/images/icons/Pay/apple-pay.svg" alt="apple-pay" />
          <img src="~assets/images/icons/Pay/google-pay.svg" alt="google-pay" />
        </div>
      </div>
    </div>
  </footer>
</template>
<style>
footer {
    background-color: #6D6875;
    color: #fff;
}
footer p, footer a{
    font-size: 14px;
    line-height: 17px;
    letter-spacing: 0.045em;
}
.footer__container {
    display: flex;
    align-items: center;
    padding: 30px 0;
}
.footer__logo, .footer__link{
    color: #fff;
}
.footer__logo {
    font-family: "SeoulHangang CBL",  serif;
    text-decoration: none;
    font-size: 36px;
    text-shadow: 0px 7px 20px rgba(0, 0, 0, 0.45);
    line-height: 36px;
    letter-spacing: 0.105em;
    margin-right: 42px;
}
.footer__copyright{
    margin-right: 29px;
}
.footer__list li:not(:last-child){
    margin-bottom: 20px;
}
.footer__list {
    list-style: none;
    margin: 0 45px 0 0;
}
.footer__list_last{
    margin: 0;
}
.footer__link {
    text-decoration: none;
    line-height: 17px;
    letter-spacing: 0.045em;
}
.footer__social {
    margin: 0 58px 0 75px;
}
.social__title{
    margin-bottom: 5px;
}
.pay__title {
    margin-bottom: 10px;
}
.social__block, .pay__block {
    display: flex;
    align-items: center;
}
.social__block img:not(:last-child), .pay__block img:not(:last-child){
    margin-right: 10px;
}
</style>